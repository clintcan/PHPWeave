...
Version 2.1 introduces the `$PW` global object, providing a cleaner and more intuitive API for accessing framework components.
...
All three methods use the same underlying lazy loading mechanism, so there's no performance difference.
...
Version 2.1 enhances the `Controller::show()` method to automatically extract array data into individual variables in views. This eliminates the need for verbose `$data['key']` syntax.
...
When passing non-array data (strings, objects, etc.), the data is still accessible via `$data`:
...
All v2.1 features are **fully backward compatible**. Existing code will continue to work without any changes:
...
For better IDE autocomplete and type hinting, you can add PHPDoc comments:
...
The `$PW` global object provides a foundation for future framework enhancements:
...
Both features maintain 100% backward compatibility, allowing gradual adoption without breaking existing code.
